...
// Set default values 
FRGrasp::FRGrasp(TMultiMap<ERHandPart, FConstraintInstance*>& FingerTypeToConstrs)
{
	...
	{
		// Get the finger type
		const ERHandPart FingerType = FingerToConstr.Key;
		// Set finger target
...
void FRGrasp::Update(const float Step)
{
	auto GraspUpdateLambda = [&](const float LambStep)
	{
		...
		{
			// Finger type
			const ERHandPart Finger = FingerToConstrs.Key;
...
void FRGrasp::BlockFinger(ERHandPart Finger)
{
	BlockedFingers.AddUnique(Finger);
}

// Free finger
void FRGrasp::FreeFinger(ERHandPart Finger)
{	
...
bool FRGrasp::IsFingerBlocked(const ERHandPart Finger)
{
